...
<div class="content <?php echo esc_attr( $instance['style'] ) ?> yith-wcwl-items-<?php echo $instance['unique_id'] ?> woocommerce wishlist-fragment on-first-load" data-fragment-options="<?php echo esc_attr( json_encode( $fragments_options ) ) ?>" >
    ...
    <div class="heading">
        ...
        <div class="items-counter">
            ...
            <span class="heading-icon">
                <?php echo apply_filters( 'yith_wcwl_widget_items_extended_heading_icon', $heading_icon ) ?>
            </span>
        ...
        <?php if( isset( $instance['style'] ) && 'extended' == $instance['style'] ): ?>
            <h3 class="heading-title"><?php echo apply_filters( 'yith_wcwl_widget_items_extended_title', 'Wishlist' ) ?></h3>
        <?php endif; ?>
    </div>

    <div class="list">
        <?php if( ! $instance['ajax_loading'] && isset( $instance['show_count'] ) && 'yes' == $instance['show_count'] && 'mini' == $instance['style'] && count( $products ) ): ?>
            <p class="items-count"><?php printf( __( '%d items in wishlist', 'yith-woocommerce-wishlist' ), count( $products ) ) ?></p>
        <?php endif; ?>
        ...
        <?php if( ! $instance['ajax_loading'] && ! empty( $products ) ): ?>
            <ul class="cart_list product_list_widget">
                ...
                ?>
                    <li>
                        <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $product_id ) ) ?>" class="remove_from_all_wishlists" data-product-id="<?php echo esc_attr( $product_id ) ?>" data-wishlist-id="<?php echo 'yes' == $instance['show_default_only'] ? $default_wishlist->get_id() : '' ?>" >&times;</a>
                        <a href="<?php echo $product->get_permalink() ?>" class="image-thumb">
                        ...
                        <div class="mini-cart-item-info">
                            ...
                            <span class="min-cart-subtotal">
                                <?php printf( '%d x %s', $info['quantity'], $product->get_price_html() ) ?>
                            </span>

                            <?php if( 'yes' != $instance['show_default_only'] ): ?>
                                <small class="mini-cart-wishlist-info">
                                <?php printf( '%s: %s', __( 'In', 'yith-woocommerce-wishlist' ), implode( ', ', $info['in_list'] ) ) ?>
                            </small>
                            <?php endif; ?>
                        </div>
                    </li>
        ...
        <?php else: ?>
            <p class="empty-wishlist">
                <?php echo apply_filters( 'yith_wcwl_widget_items_empty_list', __( 'Please, add your first item to the wishlist', 'yith-woocommerce-wishlist' ) ) ?>
            </p>
        <?php endif; ?>

        <?php if( count( $products ) && 'yes' == $instance['show_view_link'] ): ?>
            <a class="show-wishlist" href="<?php echo esc_url( $wishlist_url ) ?> "><?php _e( 'View your wishlist &rsaquo;', 'yith-woocommerce-wishlist' ) ?></a>
        <?php endif; ?>

        <?php if( count( $products ) && 'yes' == $instance['show_add_all_to_cart'] ): ?>
            <a class="btn button alt add_all_to_cart" href="<?php echo esc_url( $add_all_to_cart_url ) ?>"><?php _e( 'Add all to cart', 'yith-woocommerce-wishlist' ) ?></a>
        <?php endif; ?>
    </div>
