...
<!-- WISHLIST GRID -->
<ul class="wishlist_table wishlist_view shop_table cart images_grid responsive products columns-<?php echo esc_attr( apply_filters('yith_wcwl_wishlist_view_images_columns',wc_get_loop_prop( 'columns' ) ) ); ?>" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo $wishlist_id ?>" data-token="<?php echo $wishlist_token ?>">
	<?php
	if( $wishlist && $wishlist->has_items() ) :
		foreach( $wishlist_items as $item ) :
			...
			if( $product && $product->exists() ) :
				?>
				<li id="yith-wcwl-row-<?php echo $item->get_product_id() ?>" <?php wc_product_class( '', $product ); ?> data-row-id="<?php echo $item->get_product_id() ?>">
					<div class="item-wrapper">
						...
						<div class="item-details">
							<div class="item-details-wrapper">
                                <a class="close" href="#" title="<?php _e( 'Close', 'yith-woocommerce-wishlist' )?>">&times;</a>
								<h3 class="product-name"><?php echo apply_filters( 'woocommerce_in_cartproduct_obj_title', $product->get_title(), $product ) ?></h3>
									...
									<table class="item-details-table">
										...
										<?php if( $show_dateadded && $item->get_date_added() ): ?>
											<tr class="date-added">
												...
												<td class="value">
													<?php echo '<span class="dateadded">' . $item->get_date_added_formatted() . '</span>' ?>
												</td>
											</tr>
										<?php endif; ?>
