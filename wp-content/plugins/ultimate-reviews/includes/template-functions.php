...
function ewd_urp_reviews_shortcode( $atts ) {
	...
	$args = shortcode_atts( $review_atts, $atts );
...
function ewd_urp_review_shortcode( $atts ) {
	...
	// Extract the shortcode attributes
	$combined_atts = shortcode_atts( $review_atts, $atts );

	$review_post = get_post( $combined_atts['review_id'], ARRAY_A );

	if ( ! $review_post or $review_post['post_type'] != EWD_URP_REVIEW_POST_TYPE ) { return; }

	$args = array_merge( $combined_atts, $review_post );

	// Render review
	ewd_urp_load_view_files();
	$review = new ewdurpViewReview( $args );

	$review->single_post = true;
...
function ewd_urp_summary_statistics_shortcode( $atts ) {
	...
	$reviews->set_pagination_data( $query->get_pagination_data() );
