...
class ewdurpImport {
		...
		<div class='wrap'>
			...
			<?php } else { ?>
				<div class='ewd-urp-premium-locked'>
					<a href="https://www.etoilewebdesign.com/license-payment/?Selected=URP&Quantity=1" target="_blank">Upgrade</a> to the premium version to use this feature
				</div>
			<?php } ?>
		</div>
	<?php }
	...
	function handle_spreadsheet_upload() {
        ...
        if ( ! isset($_FILES['ewd_urp_reviews_spreadsheet']['name'] ) or ( ! preg_match("/\.(xls.?)$/", $_FILES['ewd_urp_reviews_spreadsheet']['name'] ) and ! preg_match( "/\.(csv.?)$/", $_FILES['ewd_urp_reviews_spreadsheet']['name'] ) ) ) {
	...
	public function enqueue_import_scripts() {
		...
		if ( $screen->id == 'fdm-menu_page_fdm-import' ) {
			wp_enqueue_style( 'fdm-admin', EWD_URP_PLUGIN_URL . '/assets/css/admin.css', array(), '2.0.0' );
			wp_enqueue_script( 'fdm-admin-js', EWD_URP_PLUGIN_URL . '/assets/js/admin.js', array( 'jquery' ), '2.0.0', true );
		}
	}
