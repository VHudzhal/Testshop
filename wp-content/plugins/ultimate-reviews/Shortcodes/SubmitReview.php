...
function Insert_Review_Form($atts) {
	...
	$Login_Options = is_array( get_option("EWD_URP_Login_Options") ) ? get_option("EWD_URP_Login_Options") : array();
	...
	if (isset($_REQUEST['order_id'])) {$ReturnString .= '<input type="hidden" name="order_id" value="' . esc_attr( $_REQUEST['order_id'] ) . '" />';}
	...
	if (isset($WooCommerce_Return)) {
		$ReturnString .= "<input type='hidden' name='WC_Email' value='" . esc_attr( $WooCommerce_Return['WC_Email'] ) . "' />";
	}
...
function EWD_URP_Add_Product_Name_Field($product_name, $Required) {
	...
	if ($product_name != "") {
		...
		$ReturnString .= __("Review of ", 'ultimate-reviews') . " " . esc_html( $product_name );
		$ReturnString .= "</div>";
		$ReturnString .= "<input type='hidden' name='Product_Name' value='" . esc_attr( $product_name ) ."' />";
	}
	elseif ($Product_Name_Input_Type == "Dropdown") {
		...
		$ReturnString .= "<input name='Product_Name' id='Product_Name' type='text' class='ewd-urp-product-name-text-input' value='" . (isset($_POST['Product_Name']) ? esc_attr( $_POST['Product_Name'] ) : '') . "' size='60' " . $Required_Text . "/>";
...
function EWD_URP_Add_Author_Field($Logged_In_User, $Required) {
	...
	if (!isset($Logged_In_User['Author_Name']) or $Logged_In_User['Author_Name'] == "") {
		...
		$ReturnString .= "<input type='text' name='Post_Author' id='Post_Author' value='" . (isset($_POST['Post_Author']) ? esc_attr( $_POST['Post_Author'] ) : '') . "' " . $Required_Text . "/>";
...
function EWD_URP_Add_Review_Title_Field($Required) {
	...
	$ReturnString .= "<input type='text' name='Post_Title' id='Post_Title' value='" . (isset($_POST['Post_Title']) ? esc_attr( $_POST['Post_Title'] ) : '') . "' " . $Required_Text . "/>";
...
function EWD_URP_Add_Review_Video_Field($Required) {
	...
	$ReturnString .= "<input type='URL' name='Review_Video' id='Review_Video' value='" . (isset($_POST['Review_Video']) ? esc_attr( $_POST['Review_Video'] ) : '') . "' " . $Required_Text . "/>";
...
function EWD_URP_Add_Review_Item_Field($Review_Category_Item) {
	...
	$ReturnString .= "</label>";
	if ($Review_Score_Input == "Text") {$ReturnString .= "<input type='text' id='ewd-urp-" . $Review_Category_Item['CategoryName'] . "' name='" . $Review_Category_Item['CategoryName'] . "' value='" . esc_attr( $_POST[$Review_Category_Item['CategoryName']] ) . "' " . $Required_Text . " />";}
	elseif ($Review_Score_Input == "Stars") {
		...
		$ReturnString .= "</div>";
		$ReturnString .= "<input type='text' id='ewd-urp-" . $Review_Category_Item['CategoryName'] . "' class='ewd-urp-hidden " . $Required_Text . "' name='" . $Review_Category_Item['CategoryName'] . "' value='" . (isset($_POST['Overall_Score']) ? esc_attr( $_POST['Overall_Score'] ) : '') . "' />";
	}
	else {
...
function EWD_URP_Add_Review_Body_Field() {
	...
	$ReturnString .= "</label>";
	$ReturnString .= "<textarea name='Post_Body' class='ewd-urp-review-textarea' data-textareacount='" . $Textarea_Counter ."' required>" . (isset($_POST['Post_Body']) ? esc_attr( $_POST['Post_Body'] ) : '') . "</textarea>";
...
function EWD_URP_Add_Explanation_Field($Review_Category_Item) {
	...
	$ReturnString .= "</label>";
	$ReturnString .= "<textarea name='" . $Review_Category_Item['CategoryName'] . " Description' class='ewd-urp-review-textarea'  data-textareacount='" . $Textarea_Counter ."' " . $Required_Text . ">" . esc_html( $_POST[$Review_Category_Item['CategoryName'] . " Description"] ) . "</textarea>";
	if ($Review_Character_Limit != "") {$ReturnString .= "<div class='ewd-urp-review-character-count'  id='ewd-urp-review-character-count-" . $Textarea_Counter ."'><label></label>" . __('Characters remaining:', 'ultimate-reviews') . " " . $Review_Character_Limit . "</div>";}
	$ReturnString .= "</div>";
...
function EWD_URP_WooCommerce_Customer_Login() {
	...
	elseif (isset($_POST['Customer_Email'])) {
		$Orders = $wpdb->get_results($wpdb->prepare("SELECT post_id from $wpdb->postmeta WHERE meta_key=%s AND meta_value=%s", "_billing_email", sanitize_email( $_POST['Customer_Email'] ) ));
	}
	elseif (isset($_GET['wc_customer_email'])) {
		$Orders = $wpdb->get_results($wpdb->prepare("SELECT post_id from $wpdb->postmeta WHERE meta_key=%s AND meta_value=%s", "_billing_email", sanitize_email( $_GET['wc_customer_email'] ) ));
	}
	else {
		$Orders = array();
	}
	...
	elseif (isset($_GET['wc_product_name'])) {
		foreach ($Products as $Product) {
			if ($Product['name'] == urldecode($_GET['wc_product_name'])) {
				...
				elseif (isset($_GET['wc_customer_email'])) {
					$post_id = $wpdb->get_results($wpdb->prepare("SELECT post_id from $wpdb->post_meta WHERE meta_key=%s AND meta_value=%s", "_billing_email", sanitize_email( $_POST['Customer_Email'] ) ));
		...
		foreach ($Products as $Product) {
			...
			$ReturnArray['Response_HTML'] .= "<a href='" . add_query_arg(array('wc_product_name' => $Product['name'], 'wc_customer_email' => esc_attr( $_POST['Customer_Email'] ))) ."'>" . $Product['name'] . "</a>";
			$ReturnArray['Response_HTML'] .= "</div>";
		}
		$ReturnArray['Response_HTML'] .= "</div>";
	}
