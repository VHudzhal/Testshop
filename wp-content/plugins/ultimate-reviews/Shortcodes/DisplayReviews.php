...
function EWD_URP_Display_Reviews($atts) {
	...
	if (isset($_GET['current_page'])) {$current_page = sanitize_text_field( $_GET['current_page'] );}

	if (isset($_GET['review_score'])) {$min_score = sanitize_text_field( $_GET['review_score'] ); $max_score = sanitize_text_field( $_GET['review_score'] );}

	if (isset($_GET['review_min_score'])) {$min_score = sanitize_text_field( $_GET['review_min_score'] );}

	if (isset($_GET['review_max_score'])) {$max_score = sanitize_text_field( $_GET['review_max_score'] );}

	if (isset($_GET['review_author'])) {$review_author = sanitize_text_field( $_GET['review_author'] );}
	...
	$HeaderString .= "<input type='hidden' name='product_name' value='" . esc_attr( htmlspecialchars($product_name, ENT_QUOTES) ) . "' id='urp-product-name' data-shortcodeid='" . $Shortcode_ID . "' />";
	$HeaderString .= "<input type='hidden' name='review_author' value='" . esc_attr( htmlspecialchars($review_author, ENT_QUOTES) ) . "' id='urp-review-author' data-shortcodeid='" . $Shortcode_ID . "' />";
	...
	$HeaderString .= "<input type='hidden' name='min_score' value='" . esc_attr( $min_score ) . "' id='urp-min-score' data-shortcodeid='" . $Shortcode_ID . "' />";
	$HeaderString .= "<input type='hidden' name='max_score' value='" . esc_attr( $max_score ) . "' id='urp-max-score' data-shortcodeid='" . $Shortcode_ID . "' />";
    $HeaderString .= "<input type='hidden' name='orderby' value='" . esc_attr( $orderby ) . "' id='urp-orderby' data-shortcodeid='" . $Shortcode_ID . "' />";
    $HeaderString .= "<input type='hidden' name='order' value='" . esc_attr( $order ) . "' id='urp-order' data-shortcodeid='" . $Shortcode_ID . "' />";
    $HeaderString .= "<input type='hidden' name='post_count' value='" . esc_attr( $post_count ) . "' id='urp-post-count' data-shortcodeid='" . $Shortcode_ID . "' />";
	...
	if ($Total_Reviews > $Reviews_Per_Page) {
		...
		if ($Infinite_Scroll == "Yes") {
			...
			$Pagination_String .= "<input type='hidden' name'post_count' value='" . esc_attr( $current_page ) . "' id='urp-current-page' data-shortcodeid='" . $Shortcode_ID . "' />";
			...
			$Pagination_String .= "<span class='paging-input'>" . esc_html ($current_page ) . __(' of ', 'ultimate-reviews') . "<span class='total-pages'>" . $Num_Pages . "</span></span>";
		...
		if (in_array("Author", $Review_Filtering)) {
			if (is_array($Review_Authors)) {
				...
				if (sizeOf($Unique_Review_Authors) > 1) {
					...
					$Filtering_String .= "<option value=''>" . $Filter_All_Label . "</option>";
					foreach ($Unique_Review_Authors as $Review_Author) {$Filtering_String .= "<option value='" . esc_attr( $Review_Author ) . "'>" . esc_html( $Review_Author ) . "</option>";}
					$Filtering_String .= "</select>";
					$Filtering_String .= "</div>";
				}
			}
...
function EWD_URP_Build_Summary_Statistics_String($Product_Name, $Review_Params, $Summary_Type = "", $Before_Text = "", $After_Text = "") {
	...
	if (isset($EWD_URP_Summary_Statistics_Array[$Product_Name])) {
		...
		if( $Display_Microdata == "Yes" && ($Summary_Statistics == "Full" || $Summary_Statistics == "Limited") ){
			...
			$SummaryString .= "<div itemprop='itemReviewed' itemscope itemtype='https://schema.org/" . $Items_Reviewed . "'>";
			$SummaryString .= "<meta itemprop='name' content='" . $Product_Name . "' />";
			$SummaryString .= "<div itemprop='aggregateRating' itemscope itemtype='https://schema.org/AggregateRating'>";
			$SummaryString .= "<meta itemprop='ratingValue' content='" . $Average_Score . "' />";
			$SummaryString .= "<meta itemprop='ratingCount' content='" . $Total_Reviews . "' />";
			$SummaryString .= "</div>";
			$SummaryString .= "</div>";
			$SummaryString .= "<meta itemprop='ratingValue' content='" . $Average_Score . "' />";
...
function EWD_URP_Build_Microdata_Summary($Product_Name, $Review_Params) {
	...
	$MicrodataString .= "<div itemprop='itemReviewed' itemscope itemtype='https://schema.org/" . isset( $Items_Reviewed ) ? $Items_Reviewed : '' . "'>";
	$MicrodataString .= "<meta itemprop='name' content='" . isset( $Product_Name ) ? $Product_Name : '' . "' />";
	$MicrodataString .= "<div itemprop='aggregateRating' itemscope itemtype='https://schema.org/AggregateRating'>";
	$MicrodataString .= "<meta itemprop='ratingValue' content='" . isset( $Average_Score ) ? $Average_Score : '' . "' />";
	$MicrodataString .= "<meta itemprop='ratingCount' content='" . isset( $Total_Reviews ) ? $Total_Reviews : '' . "' />";
	$MicrodataString .= "</div>";
	$MicrodataString .= "</div>";
	$MicrodataString .= "<meta itemprop='ratingValue' content='" . isset( $Average_Score ) ? $Average_Score : '' . "' />";
	$MicrodataString .= "<meta itemprop='bestRating' content='" . isset( $Maximum_Score ) ? $Maximum_Score : '5' . "' />";
  	$MicrodataString .= "<meta itemprop='reviewCount' content='" . isset( $Total_Reviews ) ? $Total_Reviews : '' . "' /'>";
	$MicrodataString .= "</div>";
