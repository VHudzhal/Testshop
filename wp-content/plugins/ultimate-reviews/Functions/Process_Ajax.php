...
function EWD_URP_Search() {
    ...
    if (isset($_POST['Shortcode_ID']) and $_POST['Shortcode_ID'] != "undefined") {$shortcode_id = sanitize_text_field( $_POST['Shortcode_ID'] );}
    else {$shortcode_id = "";}
    if (isset($_POST['Q']) and $_POST['Q'] != "undefined") {$search_string = strtolower( sanitize_text_field( $_POST['Q'] ) );}
    else {$search_string = "";}
    if (isset($_POST['product_name']) and $_POST['product_name'] != "undefined" and $_POST['product_name'] != "") {$product_name = sanitize_text_field( $_POST['product_name'] );}
    else {$product_name = "";}
    if (isset($_POST['review_author']) and $_POST['review_author'] != "undefined" and $_POST['review_author'] != "") {$review_author = sanitize_text_field( $_POST['review_author'] );}
    else {$review_author = "";}
    ...
    if (isset($_POST['min_score']) and $_POST['min_score'] != "" and $_POST['min_score'] != "undefined") {$min_score = intval( $_POST['min_score'] );}
    else {$min_score = 0;}
    if (isset($_POST['max_score']) and $_POST['max_score'] != "" and $_POST['max_score'] != "undefined") {$max_score = intval( $_POST['max_score'] );}
    else {$max_score = 1000000;}
    if (isset($_POST['orderby']) and $_POST['orderby'] != "undefined") {$orderby = sanitize_text_field( $_POST['orderby'] );}
    else {$orderby = "";}
    if (isset($_POST['order']) and $_POST['order'] != "undefined") {$order = sanitize_text_field( $_POST['order'] );}
    else {$order = "";}
    if (isset($_POST['post_count']) and $_POST['post_count'] != "" and $_POST['post_count'] != "undefined") {$post_count = intval( $_POST['post_count'] );}
    else {$post_count = -1;}
    if (isset($_POST['current_page']) and $_POST['current_page'] != "" and $_POST['current_page'] != "undefined") {$current_page = intval( $_POST['current_page'] );}
    else {$current_page = 1;}
    if (isset($_POST['only_reviews']) and $_POST['only_reviews'] != "" and $_POST['only_reviews'] != "undefined") {$only_reviews = sanitize_text_field( $_POST['only_reviews'] );}
...
function EWD_URP_Dismiss_Pointers() {   
    ...
    update_user_meta( $uid, 'dismissed_wp_pointers', implode(",", $unique_pointers) );
...
function EWD_URP_AJAX_Set_Options() {
    ...
    update_option("EWD_URP_Maximum_Score", intval( $_POST['maximum_score'] ) );
    update_option("EWD_URP_Review_Score_Input", sanitize_text_field( $_POST['review_score_input'] ) );
    update_option("EWD_URP_Review_Category", sanitize_text_field( $_POST['review_category'] ) );
