...
function EWD_URP_Save_Meta_Box_Data($post_id) {
	...
	if ($InDepth_Reviews == "No" or sizeOf($Review_Categories_Array) == 0) {
		$Overall_Score = min( round( intval( $_POST['EWD_URP_Overall_Score'] ), 2 ), $Maximum_Score );
		update_post_meta($post_id, "EWD_URP_Overall_Score", $Overall_Score);
	}
	else {
		$ReviewItems = 0;
		foreach ($Review_Categories_Array as $Review_Categories_Item) {
			$Field_Value = sanitize_text_field( $_POST['EWD_URP_' . str_replace(" ", "_", $Review_Categories_Item['CategoryName'])] );
			if ($Review_Categories_Item['CategoryType'] == "ReviewItem" or $Review_Categories_Item['CategoryType'] == "") {
				$Category_Score = min( intval( $Field_Value ), $Maximum_Score );
			...
			if ($Review_Categories_Item['ExplanationAllowed'] == "Yes") {
				$Category_Description = sanitize_textarea_field( $_POST['EWD_URP_' . str_replace(" ", "_", $Review_Categories_Item['CategoryName']) . "_Description"] );
...
function EWD_URP_Add_Review_Information($content) {
    ...
    if ($post->post_type == 'urp_review' and !isset($post->In_URP_Shortcode)) {
		...
		$Updated_View_Count = intval( get_post_meta($post->ID, 'urp_view_count', true) ) + 1;
		if ( ! is_admin() ) {update_post_meta($post->ID, 'urp_view_count', $Updated_View_Count);}
