...
function Add_URP_Reviews_From_Spreadsheet($Excel_File_Name){
   ...
   // check that the current user has the correct privileges to create posts
    ...
    if ( ! isset( $Title_Column ) ) { $Title_Column = -1; }
    if ( ! isset( $Author_Column ) ) { $Author_Column = -1; }
    if ( ! isset( $Review_Column ) ) { $Review_Column = -1; }
    if ( ! isset( $Score_Column ) ) { $Score_Column = -1; }
    if ( ! isset( $Email_Column ) ) { $Email_Column = -1; }
    if ( ! isset( $Name_Column ) ) { $Name_Column = -1; }
    if ( ! isset( $Categories_Column ) ) { $Categories_Column = -1; }
    ...
    foreach ($Data as $Review) {
        ...
        if ( ! isset( $Post_Categories) ) { $Post_Categories = array(); }
        ...
        if(!isset($_FILES['Reviews_Spreadsheet']['name']) or (!preg_match("/\.(xls.?)$/", $_FILES['Reviews_Spreadsheet']['name']) and !preg_match("/\.(csv.?)$/", $_FILES['Reviews_Spreadsheet']['name']))) {
            $error = __('File must be .csv, .xls or .xlsx', 'ultimate-reviews');
                      ...
                      $msg .= $_FILES['Reviews_Spreadsheet']['name'];
                        ...
                        $target_path = EWD_URP_CD_PLUGIN_PATH . "review-sheets/";
