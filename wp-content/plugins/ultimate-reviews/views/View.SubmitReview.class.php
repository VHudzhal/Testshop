...
class ewdurpViewSubmitReview extends ewdurpView {
	...
	public function maybe_display_captcha_field() {
		...
		$template = $this->find_template( 'submit-review-captcha' );
	...
	public function element_required() {
		...
		return $this->current_review_element->required ? 'required' : '';
	}
	...
	public function set_reviewable_product_names() {
		...
		if ( $ewd_urp_controller->settings->get_setting( 'upcp-integration' ) ) {
			...
			foreach ( $upcp_products as $upcp_product ) { $this->product_names[] = $upcp_product->Get_Product_Name(); }
		}
	...
	public function enqueue_assets() {
		...
		wp_localize_script( 'ewd-urp-js', 'ewd_urp_php_submit_review_data', $submit_review_data );
	}
